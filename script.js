...
document
  .getElementById('translate-and-analyze-btn')
  .addEventListener('click', async () => {
    ...
    if (sentimentData) {
      ...
      document.getElementById('translated-text').innerText =
        sentimentData.translatedText

      document.getElementById(
        'text-analysis-result'
      ).innerText = `Compound: ${sentimentData.sentiment.compound}, 
...
document
  .getElementById('analyze-image-btn')
  .addEventListener('click', async () => {
...
document
  ...
  .addEventListener('click', async () => {
    ...
    if (analysisResult) {
      ...
      ).innerText = `Analysis: ${analysisResult[0]['expression']}`
    } else {
  ...
  .addEventListener('submit', function (event) {
    ...
    fetch('https://sentiment-sere.onrender.com/api/analyze-image', {
      ...
      .then((data) => {
        ...
        document.getElementById(
          'result'
        ).innerText = `Expression: ${data.expression}` //Changed from emotion to expression
      })
      .catch((error) => {
  ...
  try {
    ...
    if (response.ok && data.analysis) {
      ...
      console.error(
        ...
        data.error || 'No analysis result found.'
      )
      return null
    }
  } catch (error) {
